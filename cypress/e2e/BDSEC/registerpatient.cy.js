...
context('Actions', () => {
  ...
  it('Clinical Module', () => {
    ...
    function selectRandomOptionsForAllDropdowns() {
          cy.get('.obs-control-select-wrapper') // Select all dropdown containers
            .each(($dropdown) => {
              // Open each dropdown
              ...
              cy.get('.Select-menu')
                .find('[role="option"]') // Options inside the opened dropdown
                .then((options) => {
                  const totalOptions = options.length;
                  const randomIndex = Math.floor(Math.random() * totalOptions);
                  cy.wrap(options[randomIndex]).click(); // Select a random option
                });
            });
        }
